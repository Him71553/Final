...
<div class="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:py-16">
    ...
    <div class="grid gap-8 sm:grid-cols-1">
        {#each data.posts as post, i (post.id)}
            ...
            >
                <div class="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-blue-500/10 blur-3xl transition-opacity opacity-0 group-hover:opacity-100 dark:bg-blue-400/10"></div>

                <div class="flex flex-col gap-4">
                    <div class="flex items-center gap-3 text-sm text-slate-500 dark:text-slate-400">
                        <time datetime={post.created_at} class="flex items-center gap-1.5 font-medium text-blue-600 dark:text-blue-400">
                    ...
                    <h2 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">
                        <a href={`/post/${post.id}`} class="before:absolute before:inset-0 focus:outline-none">
                            <span class="bg-gradient-to-r from-slate-900 to-slate-900 bg-[length:0%_2px] bg-left-bottom bg-no-repeat transition-all duration-300 group-hover:bg-[length:100%_2px] dark:from-blue-400 dark:to-blue-400">
                                {post.title}
                            </span>
                        </a>
                    </h2>
                    ...
                    <div class="mt-2 flex items-center pt-4 text-sm font-semibold text-blue-600 transition-colors group-hover:text-blue-700 dark:text-blue-400 dark:group-hover:text-blue-300">
                        閱讀全文
                        <svg 
    ...
    {#if data.posts.length === 0}
        <div in:fade class="flex min-h-[300px] flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-12 text-center dark:border-slate-700 dark:bg-slate-800/50">
            ...
            <p class="mt-2 text-slate-500 dark:text-slate-400">目前還沒有發布任何內容，請稍後再回來查看。</p>
        </div>
    {/if}
</div>