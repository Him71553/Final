<script lang="ts">
    ...
    import { user } from '$lib/stores'; // 引入 user
                ...
                {#if activeTab === 'write'}
                    ...
                    >
                        ...
                        <div class="flex items-center justify-end gap-3 pt-2">
                            <a 
                                href={resolve('/')}
                                class="rounded-lg px-5 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-white transition-colors"
                            >
                                取消
                            </a>
                            <button
                ...
                {:else}
                    <div in:fade={{ duration: 200 }} class="p-8 sm:p-10">
                        <article class="prose prose-slate max-w-none dark:prose-invert">
                            ...
                            <h1 class="mb-4 text-3xl font-bold text-slate-900 dark:text-white sm:text-4xl">
                                {title || '無標題'}
                            </h1>
                            ...
                            <div class="whitespace-pre-wrap text-lg leading-relaxed text-slate-700 dark:text-slate-300">
                                {content || '（無內容）'}
                            </div>
                        </article>
                    </div>
